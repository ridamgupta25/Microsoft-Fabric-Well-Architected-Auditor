...
4. Update pinned values to the new truth (never weaken a test):
   - `checks_registered == 148` in [tests/test_api.py](../../backend/tests/test_api.py) → +1 per registered check.
   ...
   A **roadmap/manual** attestation changes `checks_registered` + row counts but
