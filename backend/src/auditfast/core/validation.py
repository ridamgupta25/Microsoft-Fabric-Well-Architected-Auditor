...
VALIDATED_CHECKLIST: dict[str, str] = {
  ...
  "3.7.5": "Gold Warehouse structured to serve the model efficiently (no expensive per-query transformations)",
  ...
  "2.5.3":  "Run control tables capture batch ID, status, row counts, start/end timestamps",
    ...
    "13.2.3": "Sensitivity labels applied across Fabric items",
    "13.3.1": "Workspace is assigned to a Fabric capacity [WS-CAPACITY]",
    "13.4.1": "Pipeline activities set an explicit timeout (not Fabric's multi-day default) [PL-TIMEOUT]",
    "13.1.3": "Workspace name follows the organization naming convention [WS-NAME]",
    ...
    "6.5.1": "RLS defined on semantic models and tested per role",
    "6.5.2": "Object-Level Security applied where fields must be hidden from some audiences",
    ...
    "1.4.1": "Star schema followed in the semantic model (single-direction relationships, no unnecessary bidirectional filters)",
    "1.4.3": "Measures centralized (no duplicated calculation logic across reports)",
    "1.4.4": "DAX follows good practices (variables, no repeated sub-expressions, avoids expensive iterators)",
    ...
    "4.5.7": "Date/Time dimension exists with all required attributes (fiscal periods, quarter, holidays).",
    ...
    "4.5.9": "SCD Type 2 dimensions use canonical valid_from, valid_to, and is_current columns where the pattern is implemented.",
    ...
    "13.1.2": "Workspace item inventory captured (informational - enumerates all items; never fails) [WS-INVENTORY]",
    ...
    "9.3.3":  "Transaction boundaries defined for multi-step operations (incl. Warehouse loads)",
    ...
    "11.5.4": "Semantic models and reports are source-controlled and deployed via pipeline (Dev -> QA -> Prod)",
    ...
    "3.7.4": "Aggregations / summarizations used for performance-critical reports where needed",
    "2.7.2": "Incremental refresh configured for large Import models where applicable",
    ...
    "1.4.2": "Relationships correctly defined (cardinality, active/inactive) with no ambiguous paths",
