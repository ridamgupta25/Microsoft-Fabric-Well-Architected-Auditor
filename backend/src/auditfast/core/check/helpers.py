...
def manual(evidence: str = "Manual attestation — verify during the review",
           *, obj: str | None = None) -> Verdict:
    """A practice that cannot be judged from Fabric data — recorded, not scored.

    Checks registered with ``manual=True`` are listed in the catalog but never
    executed by the engine, so this builder is mainly for direct invocation.
    """
    return Verdict(evidence=evidence, score=None, scored=False, status=Status.NA, obj=obj)
