...
@check(
    id="WS-RLS-RPT", ref="6.5.1", title="RLS defined on semantic models and tested per role",
...
@check(
    id="WS-OLS-RPT", ref="6.5.2", title="Object-Level Security applied where fields must be hidden from some audiences",
    pillar=Pillar.SECURITY_ACCESS, scope=Scope.WORKSPACE, severity=Severity.CRITICAL,
    layers=[Layer.REPORTING], requires=[Resource.SEMANTIC_MODEL_DEFINITIONS], required=True,
)
