"""Governance & Compliance · Data Operations — roadmap (gated) checks (auto-generated).
...
_CHECKS: list[tuple[str, str, str, tuple[str, ...], bool, str]] = [
    ...
    ("R-1-3-6", "1.3.6", "All source connections inventoried (even if only in pipeline metadata)", (Layer.OPERATIONS,), True, "ITEM_DEFINITION"),
    ("R-13-1-1", "13.1.1", "Architecture overview document exists (or can be generated from workspace)", (Layer.OPERATIONS,), True, "MANUAL"),
]

for _id, _ref, _title, _layers, _required, _requirement in _CHECKS:
    check(
        id=_id, ref=_ref, title=_title,
        pillar=Pillar.DATA_GOVERNANCE, scope=Scope.WORKSPACE,
