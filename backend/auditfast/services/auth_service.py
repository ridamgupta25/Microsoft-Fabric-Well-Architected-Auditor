...
It exposes plain functions that acquire a delegated, **read-only** token via
MSAL (interactive, device-code, or by reusing an existing ``az login``) and
stashes it in an in-memory session keyed by an opaque id. Tokens live only for
the process lifetime and are never written to disk or logged.

The Flask routes in :mod:`auditfast.web.routes.auth_routes` are thin wrappers
that translate :class:`AuthError` into HTTP status codes.
"""
...
def login_interactive(email: str, tenant_id, client_id, auth_cfg: dict) -> dict:
    ...
    try:
        ...
        raise AuthError("msal is not installed", 500)
    ...
    try:
        ...
        raise AuthError(f"could not initialize sign-in: {exc}", 400)
...
def start_device_flow(tenant_id, client_id, scopes) -> dict:
    ...
    try:
        ...
        raise AuthError(f"could not start sign-in: {exc}", 400)
