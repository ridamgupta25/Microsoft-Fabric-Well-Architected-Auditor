...
## 2026-09-07 — memory ceiling, AI fetch-code execution, per-workspace fetch

- **Approved custom checks** flow into Audit Reports, Excel exports, and History with
  independent 0–100 scoring, kept separate from the deterministic 0–3 scorecard.
- **Per-workspace approval memory** — approved checks are auto-selected on later runs
  without repeated AI calls.
- **Per-workspace live fetch** for missing Fabric metadata — only the workspaces missing
  a field are fetched (evidence labelled per workspace), running read-only in a hardened
  sandbox (SSRF path screen, ≤20 calls, ≤2 MB, 5 s). Can **execute the AI-generated
  `fetch(client, workspace_id)` code** as the sole live path when enabled. OFF by default
  (`custom_checks_live_fetch_enabled`, `custom_checks_execute_fetch_code`).
- **Layered guardrails + hardened runner** — always-on regex screen (length, zero-write,
  jailbreak, injection) plus Guardrails-AI validators (length, zero-write, PII, secrets),
  with AST allow-list, restricted namespace, timeout, and a new per-check **memory ceiling**
  (`custom_checks_max_memory_mb`, 0 = off; `tracemalloc`-sampled, aborts with
  `MemoryLimitError`).
- **Qdrant** available for scalable semantic search; in-memory remains the default.
- **Fixes:** `guard.use(*validators)` for guardrails 0.11; code-gen retries gracefully on
  an LLM `ImportError` instead of a 500.
- **Optional / not active by design:** heavy ML jailbreak/injection validators (redundant
  with the regex screen — jailbreak installed but inactive under transformers 5.x/hf_hub
  1.x); LangGraph wrapper (plain Python used instead).
- **Validation:** runner 30, guardrails 28, live-provider + KB-identifier 38 tests green
  (0 failures); live fetch verified end-to-end across multiple workspaces.
