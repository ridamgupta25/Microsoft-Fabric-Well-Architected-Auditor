#!/usr/bin/env python3
r"""Standalone, client-run Microsoft Fabric workspace KB crawl CLI.
...
Optional SQL metadata: pyodbc plus Microsoft ODBC Driver 18.
...
methods remain source-compatible but the CLI's non-admin profile excludes them.
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Collect Microsoft Fabric workspace evidence into archive-compatible KB JSON. "
            "Runs independently: no AuditFAST installation, server or check engine is needed."
        ),
        epilog=(
            "Install approved dependencies: python -m pip install requests PyYAML msal\n"
            "Optional SQL metadata: python -m pip install pyodbc\n"
            "SQL also needs Microsoft ODBC Driver 18 and network access to TCP 1433.\n\n"
            "Examples:\n"
            "  python kb_crawl_cli.py --self-test\n"
            "  python kb_crawl_cli.py --auth browser\n"
            "  python kb_crawl_cli.py --auth azure-cli --list-workspaces\n"
            "  python kb_crawl_cli.py --tenant-id TENANT_UUID --auth browser\n"
...
def collect_workspaces(runtime, provider_factory, workspace_ids, layer, output_dir, *, tokens=None):
    ...
    requested = workspace_resources(runtime)
    excluded = frozenset(runtime.Resource) - requested
    run_dir = _new_run_directory(Path(output_dir))
    summary = {
    ...
    for index, workspace_id in enumerate(workspace_ids, start=1):
        ...
        except runtime.WorkspaceAccessError as exc:
            failures += 1
            summary["workspaces"].append({
                "workspace_id": workspace_id,
                "status": "failed",
                "http_status": exc.status,
                "reason": "Workspace could not be read; verify access, identity and connectivity.",
                "capture_started_at": started,
                "capture_finished_at": _utc_now(),
            })
            print(f"  Failed: workspace access/read problem (HTTP {exc.status or 'unavailable'}).")
        else:
...
def main(argv=None):
    ...
    try:
        ...
        if importlib.util.find_spec("pyodbc") is None:
            print("WARNING: pyodbc is absent; SQL metadata coverage will be unavailable.")
        code, _summary = collect_workspaces(
            runtime, lambda: _create_provider(runtime, tokens, args.timeout),
